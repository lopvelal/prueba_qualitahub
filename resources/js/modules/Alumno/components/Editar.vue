...
const obtenerAlumnos = async () => {
    try {
        const response = await axios.get(`/api/alumno/${id}`);
...
// Cargar los datos del alumno cuando el componente se monte
onMounted(async () => {
...
const guardarCambios = async () => {
    try {
        await axios.put(`/api/alumno/${id}`, alumno.value);
        alert('Alumno actualizado correctamente');
        router.push({ name: 'Alumnos' })
        // Aquí podrías redirigir al usuario o actualizar el estado de la UI según sea necesario
    } catch (error) {
